...
- [ ] **SDK init 자동화** — `sdk.call()` 이 처음 불릴 때 `CAFE24API.init` 을 한 번 부른다.
      ...
      `CLIENT_ID` 가 비어 있는 몰에서 init 이 실패해도 회원이 유지되는지(판단 불가로 미룬다 — 코드상 확인됨)
- [ ] **비회원 응답 모양** — `getCustomerInfo` 가 비회원일 때 콜백 에러로 오는지, 정상 응답의 `error.code: 403` 으로 오는지.
      키트는 둘 다 비회원으로 본다(허닭이 둘 다 처리). 네트워크 오류도 지금은 콜백 에러라 비회원으로 보인다 — 실제로 어떻게 오는지 확인
